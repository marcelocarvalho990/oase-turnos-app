...
function Spinner() {
  return (
    <div className="flex flex-col items-center justify-center py-32 gap-3">
      ...
      <span className="text-sm text-slate-500">A carregar definições…</span>
    </div>
  )
}
...
export default function DefinicoesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!settingsRes.ok) throw new Error('Erro ao carregar definições')
      if (!staffRes.ok) throw new Error('Erro ao carregar colaboradores')
  ...
  async function saveSettings() {
    ...
    try {
      ...
      setSettings(updated)
      setSuccessMsg('Definições guardadas com sucesso.')
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro desconhecido')
    } finally {
      setSaving(false)
  ...
  async function saveEmployeeVacDays(empId: string) {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Erro ao guardar dias de férias')
      setSuccessMsg(`Dias de férias atualizados.`)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro desconhecido')
    } finally {
      setSavingEmpId(null)
        ...
        {loading ? <Spinner /> : (
          <>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
              <h2 className="text-base font-semibold text-slate-800">Configurações da equipa</h2>

              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-700">
                  Horas mensais base (100%)
                </label>
                <p className="text-xs text-slate-400 -mt-1">
                  Usado para calcular o objetivo de horas de cada colaborador. Por defeito: 160h.
                </p>
                <div className="flex items-center gap-3">
                  <input
                    type="number"
                  ...
                  />
                  <span className="text-sm text-slate-500">horas / mês</span>
                  <button
              ...
              {settings && (
                <p className="text-xs text-slate-400">
                  Último valor guardado: <span className="font-mono font-medium">{settings.baseMonthlyHours}h</span>
                </p>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100">
                <h2 className="text-base font-semibold text-slate-800">Dias de férias por colaborador</h2>
                <p className="text-xs text-slate-400 mt-0.5">
                  Direito anual base (100%). Usado no cálculo de férias proporcionais.
                </p>
              </div>
              ...
              {employees.length === 0 ? (
                ...
                <div className="divide-y divide-slate-50">
                  ...
                  <div className="grid grid-cols-[1fr_80px_100px_100px] gap-3 px-6 py-2 bg-slate-50/70 text-xs font-bold uppercase tracking-wider text-slate-500">
                    ...
                    <span className="text-center">Dias férias</span>
                    <span />
                  </div>
